...
function startCasting() {
  return new Promise((resolve, reject) => {
    ...
    castProcess = spawn('ffmpeg', [
      '-f', 'android_camera', '-i', '0', '-f', 'mpegts',
      '-vcodec', 'mpeg1video', '-s', '1280x720', '-b:v', '1000k',
      '-r', '30', '-bf', '0', '-codec:v', 'mpeg1video',
      `http://${ip}:8082/screen`
    ]);
    castProcess.on('error', reject);
    isCasting = true;
...
<div class="cast-screen"><canvas id="castCanvas" class="cast-video"></canvas></div>
...
if(data.active)document.getElementById('castUrl').textContent='Cast URL: '+data.url;
}catch(e){}
}
...
if(status.active)document.getElementById('castUrl').textContent='Cast URL: '+status.url;
});
